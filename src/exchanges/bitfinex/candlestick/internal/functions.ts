...
export function getCandleStickUrl(pair: string, minutesFoot: number, start?: number, end?: number, limit?: number, sort: number = -1): string {
  ...
  if (start && end && !limit) {
    limitAdjusted = Math.min(Math.round((end - start) / (minutesFoot * 60 * 1000)) + 1, 1000);
    startAdjusted = 0;
  }

  if (startAdjusted) {
    url += `&start=${startAdjusted}`;
  }
  if (end) {
    url += `&end=${end}`;
  }
